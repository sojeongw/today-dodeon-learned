...
하지만 웹이 발달하고 빅데이터가 등장하면서 많은 데이터를 처리하는데 드는 비용이 매우 커지게 되었다. 데이터와 트래픽이 늘어날 수록 한 대에서 실행되도록 설계된 RDB는 하드웨어적으로 큰 비용을 요구했다. 장비의 성능이 좋을 수록 [스케일 업](../database/scaling.md) 하는 비용이 기하급수적으로 증가하기 때문이다.
