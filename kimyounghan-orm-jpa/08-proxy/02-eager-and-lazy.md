...
public class App {
    public void printUserAndTeam(String memberId) {
        ...
        // 단순히 m.getTeam()은 프록시로 가져오는 거기 때문에 이것만 해서는 Entity로 초기화되지 않는다.
...
2. 일단 Member를 가지고 온 다음 EAGER로 되어있는 Entity를 확인해서 한 번 더 쿼리를 날린다. 즉, Member 조회 + Team 조회로 두 번의 쿼리를 보낸다.
    - 성능이 좋지 않다.
