...
데이터베이스 조회를 미루는 가짜(프록시) Entity 객체를 조회한다. DB에 쿼리가 안 날아가는데 객체가 조회되는 것이다.
...
- 프록시 객체를 초기화 할 때, 프록시 객체가 실제 Entity로 바뀌는 것은 아니다.
    ...
    - 초기화 되면 프록시 객체를 통해서 실제 Entity에 접근할 수 있다.
...
- 프록시 객체는 원본 Entity를 상속받는다. 즉, 프록시인 멤버와 아닌 멤버가 타입이 맞지 않는다.
    - 따라서 타입 체크 시 주의해야 한다.
...
- 영속성 컨텍스트가 이미 있는 Entity를 찾게 되면 `em.getReference()`를 호출해도 실제 Entity를 반환한다.
