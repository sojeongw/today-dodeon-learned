...
집합은 하나의 연산으로 취급되는 연관된 객체의 모음이다. RDB처럼 ACID 트랜잭션을 한 Entity에 대해 지원하진 않지만 하나의 집합을 연산할 때는 지원한다.
...
* JOIN 연산이 불가능하다.
  * 이 때문에 MongoDB나 Cassandra 등의 DB에서 맵 리듀스 기능을 제공해 JOIN과 유사한 기능을 제공한다. 하지만 사용법이 어렵다고 한다.
...
관계가 중요한 모델이므로 탐색의 키가 데이터 간의 관계일 때 적합하다. 예를 들어, 페이스북이나 트위터에서 내 친구의 친구를 찾는 질의를 할 때나 연관된 데이터를 추천해주는 추천 엔진, 패턴 인식에 유용하다.
...
실제로 페이스북과 트위터 같은 SNS는 게시글을 저장할 때 NoSQL을 쓴다고 한다. 매 초에 수백 기가에서 테라 바이트까지 만들어지는 대용량의 데이터를 빠른 시간 안에 처리해야 하기 때문이다. 만약 NoSQL을 사용하지 않는다면 우리는 글 하나 작성할 때마다 한참을 기다려야 할지 모른다. 검색 엔진도 만약 RDB를 사용했다면 단어를 입력할 때마다 상당한 시간이 걸릴 것이다.
...
Facebook이 만든 자바 기반의 분산 데이터 베이스

#### 장점

- 칼럼 단위로 관리되므로 동일한 데이터를 행 단위보다 디스크에 많이 저장할 수 있음
- 아파치 재단에서 개발 중이며 커뮤니티가 활발함
- 대량의 읽기/쓰기가 발생하는 서비스에 최적
    - 여러 장소에 분산된 다수의 저렴한 서버를 이용해 대량의 데이터를 처리함
    - 데이터를 일단 메모리에 저장한 후 일정 크기가 되면 디스크로 이동함
- 대이터 일관성 대신 서비스 제공에 중점을 두어 가용성이 뛰어남
    - 동일한 데이터가 여러 서버에 복제되어 있어 단위 서버에 문제가 발생해도 가용성을 보장함
    - 따라서 단일 노드에 장애가 발생해도 서비스에 큰 영향을 미치지 않
- Consistent Hashing 방식에 로드 밸런싱을 위한 알고리즘을 더해 데이터를 분산함
    - 요청이 적은 서버를 요청이 많은 서버 구간으로 옮겨 로드 밸런싱을 구현함
    - 따라서 대량의 요청이 소수의 서버에 몰리는 것을 막아 장애를 방지하고 성능을 높임
    - 신규 서버 추가 시 로드 밸런싱을 고려해 요청이 많은 구간에 우선 추가하여 부하 관리를 효율적으로 함
- 기존 서비스를 중단하지 않고 새 서버를 추가할 수 있음
    - 중앙에서 별도로 관리하는 서버가 없어 새로 추가되는 서버를 가동만 하면 서비스에 초가됨

#### 단점

- 칼럼형 데이터베이스라 Row형인 RDB와 다르게 생소해 진입 장벽이 높음
    - 많은 사용자를 대상으로 대량의 데이터를 다루지 않는 경우 사용할 필요 없음
- 복잡한 조건으로 검색이 불가능함
    - Row 키와 column에 대한 인덱스만 가능하므로 대량의 데이터이지만 검색은 단순한 서비스에 적합
- 분산된 데이터 사이의 정합성 문제가 있어 RDB에 비해 신뢰도가 떨어짐
    - 데이터 입력, 변경이 비동기로 이루어지고 중앙에서 관리하는 서버가 별로 없음
    - 보완책으로 데이터 읽기 요청이 발생하면 해당 데이터를 가진 여러 서버에 요청을 보내 결과값을 비교한 뒤 time stamp가 최신 결과를 반환함
    - time stamp가 오래된 응답을 보낸 서버는 최신 데이터로 갱신하도록 함
    
#### 사용 사례
- 대규모 검색 사이트
    - 사용자가 많고 실시간으로 기록되는 데이터(검색 시도)가 많음
    - 데이터는 대부분 텍스트 데이터로 이루어짐
- RDB의 경우 1000만명이 하루 12시간, 시간당 20만 건을 검색하면 1,000,0000 x 20 x 12, 즉 24억 행의 데이터를 관리해야 함
- 고가의 장비를 도입해 RDB를 관리한다 해도 테이블 구조가 간단하고 데이터 일관성이나 join 등 복잡한 연산이 필요한 서비스가 아니므로 낭비가 됨
- Cassandra 사용 시 저비용으로 대량의 데이터를 유연하게 관리할 수 있음. 검색 정보는 실시간으로 정확하게 제공해야 하는 서비스가 아니기 때문
...
- 스키마를 정의할 필요가 없음
    - 따라서 프로그램 코드만 수정하면 됨
    - 스키마와 프로그램 코드 사이의 정합성에 신경쓰지 않아도 됨
- RDB처럼 임의의 key값에 대해 복잡한 검색이 가능함
- 비동기식으로 처리하여 응답 속도가 빠름
- 인덱스를 추가해 처리 속도를 더 빠르게 할 수 있음

#### 단점

- JOIN이나 트랜잭션 처리가 불가능함
- 디스크에 쓰기가 비동기식으로 이루어지므로 데이터 유실의 가능성이 있음
    - 데이터 갱신 및 입력 시 바로 디스크에 쓰지 않기 때문